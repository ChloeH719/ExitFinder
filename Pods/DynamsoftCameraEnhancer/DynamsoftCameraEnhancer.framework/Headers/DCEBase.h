...
typedef NS_ENUM(NSInteger, EnumResolution)
{
    ...
    /** Set the video streaming to high resolution (defined by iOS system).*/
    EnumRESOLUTION_HIGH NS_SWIFT_NAME(EnumRESOLUTION_HIGH)   = 6,

    /** Set the video streaming to mid resolution (defined by iOS system).*/
    EnumRESOLUTION_MID NS_SWIFT_NAME(EnumRESOLUTION_MID)     = 7,

    /** Set the video streaming to low resolution (defined by iOS system).*/
...
//- (instancetype)initWithRegion:(NSInteger)regionTop left:(NSInteger)regionLeft right:(NSInteger)regionRight bottom:(NSInteger)regionBottom percentage:(NSInteger)percentage;
...
/** The orientation of the image. Normally, mobile frames are rotated 90 degrees counterclockwise*/
...
- (instancetype)initWithBuffer:(NSData*)imageData width:(NSInteger)width height:(NSInteger)height stride:(NSInteger)stride pixelFormat:(NSInteger)pixelFormat frameID:(NSInteger)frameID orientation:(NSInteger)orientation quality:(EnumFrameQuality)quality isCropped:(BOOL)isCropped cropRegion:(CGRect)cropRegion imageBuffer:(CVImageBufferRef)imageBuffer;
@end
